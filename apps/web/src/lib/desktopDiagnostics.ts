...
export function buildDesktopDiagnosticsText({
  ...
  const lines = [
    ...
    `- OCR base URL: ${diagnosticValue(settings?.ocrBaseUrl)}`,
    `- OCR model: ${diagnosticValue(settings?.ocrModel)}`,
    `- OCR key: ${settings?.ocrApiKeyConfigured ? "configured server-side" : "not configured"}`,
