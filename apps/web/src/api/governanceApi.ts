...
export type ReviewPolicyPayload = Pick<
  ReviewPolicy,
  "assignedReviewerIds" | "approvalThreshold" | "requiresAssignedReviewer"
>;
...
export async function createGovernanceRecord(payload: GovernancePayload): Promise<GovernanceRecord> {
  return actorJsonRequest<GovernanceRecord>(
    "elder",
    "/api/governance",
    {
      method: "POST",
...
export async function updateReviewPolicy(languageId: string, payload: ReviewPolicyPayload): Promise<ReviewPolicy> {
  return actorJsonRequest<ReviewPolicy>(
    "reviewer",
    `/api/languages/${encodeURIComponent(languageId)}/review-policy`,
    {
      method: "PUT",
      body: JSON.stringify(payload)
    },
...
export async function fetchReviewDispositions(languageId: string): Promise<ReviewDisposition[]> {
  return getJson<ReviewDisposition[]>(`/languages/${encodeURIComponent(languageId)}/review-dispositions`, "reviewer");
}
...
export async function resolveReviewDisposition(
  ...
  return actorJsonRequest<ReviewDisposition>(
    "reviewer",
    "/api/review-dispositions/resolve",
    {
      method: "PATCH",
