...
export type OpenAiChatCompletionMessage = NonNullable<
  NonNullable<OpenAiChatCompletionResponse["choices"]>[number]["message"]
>;

export type OpenAiAssistantContentResult = { ok: true; content: string } | { ok: false; error: string };
...
export function hasReasoningOnlyContent(message: OpenAiChatCompletionMessage | undefined): boolean {
  ...
  return (
    parseAssistantMessageContent(message.content) === undefined &&
...
export function parseOpenAiChatCompletionContent(
  ...
  return {
    ok: false,
    error: hasReasoningOnlyContent(message) ? REASONING_ONLY_MESSAGE : emptyContentMessage
  };
}
