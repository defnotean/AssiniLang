...
describe("parseModelJson", () => {
  ...
  it("parses a fenced JSON object", () => {
    const result = parseModelJson(["```json", '{"summary":"from fence","count":3}', "```"].join("\n"));

    expect(result).toEqual({ summary: "from fence", count: 3 });
  });

  it("extracts the first JSON object from surrounding prose", () => {
    const result = parseModelJson('Here is the result: {"summary":"from prose","count":4} Thanks.');

    expect(result).toEqual({ summary: "from prose", count: 4 });
  });

  it("handles nested objects and braces inside strings", () => {
    const result = parseModelJson('prefix {"summary":"literal { brace } text","meta":{"count":5}} suffix');
  ...
  it("skips an invalid brace-delimited draft before a valid final object", () => {
    expect(parseModelJson('Draft notation {target = translation}; final: {"summary":"usable","count":6}')).toEqual({
      summary: "usable",
      count: 6
    });
  });
