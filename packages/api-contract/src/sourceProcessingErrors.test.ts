...
describe("sourceProcessingErrorI18n", () => {
  ...
  it("classifies unreadable OCR output", () => {
    expect(sourceProcessingErrorI18n("OCR found no readable text in the image.")).toEqual({
      i18nKey: "ingest.ocrNoReadableText"
    });
  });
  ...
  it("classifies OCR model endpoint empty or invalid responses", () => {
    expect(sourceProcessingErrorI18n("OCR model endpoint returned no text.")).toEqual({
      i18nKey: "ingest.ocrModelFailed"
    });
    expect(sourceProcessingErrorI18n("OCR model endpoint returned invalid JSON.")).toEqual({
      i18nKey: "ingest.ocrModelFailed"
    });
    expect(sourceProcessingErrorI18n("OCR model endpoint returned no choices.")).toEqual({
      i18nKey: "ingest.ocrModelFailed"
    });
    expect(sourceProcessingErrorI18n("OCR model request failed with status 502.")).toEqual({
      i18nKey: "ingest.ocrModelFailed"
    });
    expect(sourceProcessingErrorI18n("OCR model request failed: fetch failed")).toEqual({
      i18nKey: "ingest.ocrModelFailed"
    });
  });
  ...
  it("classifies empty resolved source text", () => {
    expect(sourceProcessingErrorI18n("Source contains no readable text.")).toEqual({
      i18nKey: "ingest.sourceNoReadableText"
    });
  });
  ...
  it("classifies already-processing conflicts", () => {
    expect(sourceProcessingErrorI18n("Source is already processing: src-1")).toEqual({
      i18nKey: "ingest.sourceAlreadyProcessing"
    });
  });

  it("classifies restart-recovery interruptions", () => {
    expect(sourceProcessingErrorI18n("Processing interrupted by a server restart. Re-run processing.")).toEqual({
      i18nKey: "ingest.processingInterruptedByRestart"
    });
  });

  it("classifies stale-heartbeat recovery interruptions", () => {
    expect(sourceProcessingErrorI18n("Processing stalled without progress. Re-run processing.")).toEqual({
      i18nKey: "ingest.processingStalledWithoutProgress"
    });

    expect(sourceProcessingErrorI18n("Queued source processing was cancelled. Use Retry when ready.")).toEqual({
      i18nKey: "ingest.sourceProcessingCancelled"
    });

    expect(sourceProcessingErrorI18n("Queued source processing was cancelled. Re-run processing when ready.")).toEqual({
      i18nKey: "ingest.sourceProcessingCancelled"
    });
  });

  it("classifies source URL fetch failures with status", () => {
    expect(sourceProcessingErrorI18n("Fetching source URL failed with status 404.")).toEqual({
  ...
  it("classifies oversized or empty source URL content", () => {
    expect(sourceProcessingErrorI18n("Source URL content is too large to process locally.")).toEqual({
      i18nKey: "ingest.urlContentTooLarge"
    });
    expect(sourceProcessingErrorI18n("Source URL returned no readable text content.")).toEqual({
      i18nKey: "ingest.urlNoReadableText"
    });
  });
  ...
  it("classifies transcription endpoint failures", () => {
    expect(sourceProcessingErrorI18n("Transcription request failed with status 502.")).toEqual({
      i18nKey: "ingest.transcribeFailed"
    });
    expect(sourceProcessingErrorI18n("Transcription request failed: [redacted-secret]")).toEqual({
      i18nKey: "ingest.transcribeFailed"
    });
    expect(sourceProcessingErrorI18n("Transcription endpoint returned no text.")).toEqual({
      i18nKey: "ingest.transcribeFailed"
    });
    expect(sourceProcessingErrorI18n("Transcription endpoint returned invalid JSON.")).toEqual({
      i18nKey: "ingest.transcribeFailed"
    });
  });
...
describe("sourceProcessingWarningI18n", () => {
  ...
  it("classifies scanned PDF OCR success notices", () => {
    expect(sourceProcessingWarningI18n("Used configured OCR model to read scanned document (3 of 3 pages).")).toEqual({
  ...
  it("classifies legacy page-1 scanned PDF OCR success notices", () => {
    expect(sourceProcessingWarningI18n("Used configured OCR model to read scanned document (page 1).")).toEqual({
      i18nKey: "ingest.ocrPdfUsed",
  ...
  it("classifies per-page OCR soft-fail warnings", () => {
    expect(sourceProcessingWarningI18n("OCR failed for page 2; continuing with remaining pages.")).toEqual({
      i18nKey: "ingest.ocrPdfPageFailed",
      i18nParams: { page: 2 }
    });
    expect(sourceProcessingWarningI18n("OCR skipped page 1 (no embedded page image).")).toEqual({
  ...
  it("classifies image OCR success notices", () => {
    expect(sourceProcessingWarningI18n("Used configured OCR model to read the image.")).toEqual({
      i18nKey: "ingest.ocrImageUsed"
    });
  });

  it("classifies deterministic offline-heuristic notices", () => {
    expect(
      sourceProcessingWarningI18n("No model configured (deterministic mode); used offline heuristic parsing.")
    ).toEqual({ i18nKey: "ingest.warningDeterministicHeuristic" });
  });

  it("classifies model JSON fallback notices", () => {
    expect(
      sourceProcessingWarningI18n("Model output was not valid extraction JSON; fell back to offline heuristics.")
    ).toEqual({ i18nKey: "ingest.warningOfflineHeuristicFallback" });
  });

  it("classifies local tesseract fallback notices", () => {
    expect(
      sourceProcessingWarningI18n("No vision model configured; used local OCR (tesseract.js) to read the image.")
    ).toEqual({ i18nKey: "ingest.warningLocalOcrUsed" });
  });

  it("classifies per-chunk parse skips with part counts", () => {
    expect(
      sourceProcessingWarningI18n("Model output for part 2 of 5 was not valid extraction JSON; that part was skipped.")
    ).toEqual({
  ...
  it("classifies Obsidian vault import file-limit notices", () => {
    expect(sourceProcessingWarningI18n("Import stopped at the configured 100 file limit.")).toEqual({
...
describe("vaultImportSkipReasonI18n", () => {
  it("classifies oversized and empty vault Markdown skip reasons", () => {
    expect(vaultImportSkipReasonI18n("Markdown file is larger than the 1 MB import limit.")).toEqual({
      i18nKey: "ingest.vaultMarkdownTooLarge"
    });
    expect(vaultImportSkipReasonI18n("Markdown file had no importable text.")).toEqual({
      i18nKey: "ingest.vaultMarkdownEmpty"
    });
