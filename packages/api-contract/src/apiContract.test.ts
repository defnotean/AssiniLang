...
describe("api contract schemas", () => {
  it("defaults omitted language typology to unknown", () => {
    expect(
      languageCreatePayloadSchema.parse({
        name: "  Bisaya  ",
  ...
  it("defaults omitted AI session seed prompt to an empty string", () => {
    expect(
      createAiSessionPayloadSchema.parse({
        languageId: "bisaya",
  ...
  it("trims language phonology payloads and rejects blank inventory values", () => {
    ...
    expect(
      languageCreatePayloadSchema.safeParse({
        name: "A",
        description: "B",
        orthography: "C",
  ...
  it("trims source titles and URLs without altering raw text bodies", () => {
    expect(
      sourceRegistrationPayloadSchema.parse({
        kind: "wordlist",
        title: " Field notes ",
        rawText: " mira = river \n"
      })
    ...
    expect(
      sourceRegistrationPayloadSchema.safeParse({
        kind: "url",
        title: "File",
  ...
  it("defaults Obsidian vault import options and rejects invalid limits", () => {
    expect(
      obsidianVaultImportPayloadSchema.parse({
    ...
    expect(
      obsidianVaultImportPayloadSchema.safeParse({
        vaultPath: "C:/vaults/notes",
        maxFiles: 0
      }).success
    ).toBe(false);
  ...
  it("trims elder correction targets and requires at least one target", () => {
    expect(
      elderCorrectionPayloadSchema.parse({
        languageId: " bisaya ",
        noteId: " note-1 ",
    ...
    expect(
      elderCorrectionPayloadSchema.safeParse({
        languageId: "bisaya",
        correction: "Prefer mira.",
        rationale: "Elder preference.",
        severity: "minor"
  ...
  it("rejects blank AI session context ids", () => {
    expect(
      createAiSessionPayloadSchema.safeParse({
        languageId: "bisaya",
        mode: "learner_practice",
        contextNoteIds: [" "]
      }).success
  ...
  it("trims exercise submission answers and rejects blanks or unknown keys", () => {
    ...
    expect(
      exerciseSubmissionPayloadSchema.safeParse({
        answer: "mira",
        extra: true
      }).success
    ).toBe(false);
  ...
  it("accepts process-source responses with asset drafts and warnings", () => {
    ...
    expect(
      processSourceResponseSchema.parse({
        asset,
        drafts: [draft],
        warnings: ["ok"]
      })
    ...
    expect(
      processSourceResponseSchema.safeParse({
        asset,
        drafts: [draft]
      }).success
    ).toBe(false);
    ...
    expect(
      processSourceResponseSchema.safeParse({
        asset: { ...asset, filePath: "assets/private.txt", rawText: "private" },
        drafts: [draft],
        warnings: []
      }).success
    ).toBe(false);
  ...
  it("accepts Obsidian vault import response summaries", () => {
    expect(
      obsidianVaultImportResponseSchema.parse({
        imported: [],
  ...
  it("validates AI message, prototype session, and API error envelopes", () => {
    ...
    expect(apiErrorEnvelopeSchema.safeParse({ i18nKey: "errors.languageNotFound" }).success).toBe(false);
  });

  it("dedupes bulk-review draft ids and enforces the max batch size", () => {
    expect(
      bulkReviewPayloadSchema.parse({
        action: "accept",
        draftIds: [" d1 ", "d1", "d2"]
      })
    ).toEqual({
    ...
    expect(
      bulkReviewPayloadSchema.safeParse({
        action: "noop",
        draftIds: ["d1"]
      }).success
    ).toBe(false);
  ...
  it("validates governance and review-policy payloads", () => {
    ...
    expect(
      governancePayloadSchema.safeParse({
        languageId: "bisaya",
        policyType: "consent",
        content: "ok",
    ...
    expect(
      reviewPolicyPayloadSchema.parse({
    ...
    expect(
      reviewDispositionResolvePayloadSchema.parse({
        resolutionSummary: " Done. "
      })
    ).toEqual({ resolutionSummary: "Done." });

    expect(
      reviewDispositionResolveByIdPayloadSchema.parse({
...
describe("llm contract schemas", () => {
  it("requires a profile name and accepts optional activation", () => {
    expect(
      modelProfileSavePayloadSchema.parse({
        name: " Local Ollama ",
        provider: "ollama",
        activate: true
      })
    ...
    expect(
      modelProfileSavePayloadSchema.safeParse({
        provider: "ollama"
      }).success
    ).toBe(false);
  });
  ...
  it("accepts LLM reachability and discovery response shapes", () => {
    ...
    expect(
      llmReachabilitySchema.safeParse({
        reachable: true,
        mode: "local"
      }).success
    ).toBe(false);
    ...
    expect(
      llmModelDiscoveryResponseSchema.parse({
