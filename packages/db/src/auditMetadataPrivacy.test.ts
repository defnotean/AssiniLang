...
describe("auditMetadataPrivacyIssue", () => {
  ...
  it("blocks common credential field names and transcription key patterns", () => {
    ...
    expect(auditMetadataPrivacyIssue("ASSINI_TRANSCRIBE_API_KEY=local-transcribe-key"))
      .toBe("secret-like value at metadata");
    expect(auditMetadataPrivacyIssue("ASSINI_OCR_API_KEY=local-ocr-key"))
      .toBe("secret-like value at metadata");
  });
});
