...
1. Create a language. The web console's New language form (at the bottom of the language sidebar) collects name, typology, description, and orthography. Use the phonology inventory editor on Start after creation to add consonants, vowels, stress, and syllable templates.
...
- Register a pasted text, word list, or URL source; import local Obsidian Markdown notes; browse and select text notes exposed by a configured Obsidian MCP server; or upload a file (PDF, DOCX, plain-text documents, images, or audio; 25 MB cap).
- Obsidian import accepts a vault folder path, optional subfolder traversal, and a maximum Markdown-file count. The server skips `.obsidian`, `.git`, and `node_modules`, strips common frontmatter and wikilinks, then stores each readable note as an ordinary pending text source for the same processing and review flow. This is a one-way, point-in-time filesystem import; it is not an Obsidian MCP server, live vault sync, or write-back integration.
- The MCP path is configured and tested in Settings. Build loads the server's resource list and imports only selected text resources (up to 50 per request). Resource URIs are retained for provenance and duplicate detection. This first bridge is read-only Streamable HTTP resource intake: it does not spawn stdio commands, invoke tools, mirror deletions, resolve conflicts, synchronize incrementally, or write back to the vault.
