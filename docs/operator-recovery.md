...
Writes a validated copy of the **database file only** to `data/backups/local-db-<timestamp>.json` (or the **file** path you pass — an existing directory is rejected with a clear error). Uploaded files under `data/assets/` are not included; copy that folder yourself or use a desktop folder backup if you need assets restored with the workspace. `--dry-run` resolves the source and destination paths and prints them without writing a file after confirming the live database parses; if the destination already exists, dry-run still succeeds and warns that a real write would need `--force`. The CLI refuses to write onto the live database path (including symlink or hard-link aliases of that file, Windows case-only path aliases such as `LOCAL-DB.JSON` vs `local-db.json`, and `\\?\` extended-length prefixes of the same path — `--force` does not bypass same-file identity), refuses to overwrite an existing backup file unless you pass `--force`, and refuses to archive an invalid workspace. After a successful write, the CLI prints a pasteable restore recipe with the live database path and backup path filled in (there is no `npm run db:restore` script). Restore is deliberate: call `JsonStore.restoreFrom(sourcePath)` from a Node REPL or small script so a bad backup cannot silently replace live data. Restore validates against the current schema before replacing the live database, refuses when the backup path is a directory, and also refuses when the backup path is the live database (or a symlink/hard-link to it). AssiniLang Desktop's restore-latest action validates the backup database the same way, creates a `backup-safety-before-restore-…` copy first, and refuses to wipe live data if that safety backup fails (see Desktop app below).
...
- **Create backup** — timestamped folder copy of the data directory (including `data/assets/` when present) plus optional settings before experiments. Create validates the live workspace first (same schema check as CLI `db:backup`) and refuses to archive an invalid or missing database.
- **Restore latest backup** — confirms, then replaces live data from the newest **routine** restorable backup (not a just-created `backup-safety-before-restore-…` copy, so a second restore does not undo the first). Restore validates the backup database first and refuses when `backup-manifest.json` names a database file that is missing under `data/` (no silent fallback to `local-db.*`). It then creates a restorable safety backup (`backup-safety-before-restore-…`) and **refuses to wipe live data if that safety backup fails**. Safety copies remain restorable when they are the only backup available.
- **Open backups folder** / **Prune old backups** — keeps the newest five routine backups; safety restore backups stay restorable and are left alone by prune.
