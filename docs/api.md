...
During local JSON reads, restored notes and note answer keys must reference existing languages, keep `evidenceCount` aligned with `evidencePassageIds`, keep reviewer and edit-history timestamps parseable when present, cite existing same-language evidence passages, and keep examples synchronized with the cited corpus target text and translation.
...
Persisted review-disposition records must reference an existing same-language note, use assignable local users for assignee/opener/resolver fields, keep parseable timeline fields (`dueAt`, `openedAt`, and `resolvedAt` when present), and keep open versus resolved fields internally consistent. Open disposition work must be attached to a note currently in a disposition status: `contested`, `rejected`, `deferred`, or `escalated`.
...
The persisted app-state schema also enforces one approval per language, note, and reviewer. Approval records must reference an existing same-language note that is still `under_review` or already `approved`, and they must use a known assignable reviewer. This keeps malformed local JSON from double-counting, misattributing, or reviving inactive quorum decisions.
...
Persisted AI sessions are validated during local JSON reads. Each session must reference an existing language, use a creator whose local role is allowed for the session mode, keep parseable `createdAt`, `updatedAt`, and message timestamps in chronological order, and keep all context note and corpus passage IDs within the same language.
