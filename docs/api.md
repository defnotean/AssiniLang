...
| POST | `/auth/prototype-session` | Public (requires `ASSINI_ENABLE_PROTOTYPE_AUTH=true`; learner/elder/reviewer/programmer users only) | Open a local HTTP-only prototype session. Sessions expire after `ASSINI_PROTOTYPE_SESSION_TTL_MS` (default 8 hours) with sliding renewal on use (server `expiresAt` refreshes, and cookie `Max-Age` is re-issued to the **remaining** lifetime — sliding window ∩ `ASSINI_PROTOTYPE_SESSION_ABSOLUTE_MAX_MS`, not a full TTL reset near the absolute deadline). Absolute max defaults to 24 hours / 3× TTL from `createdAt`, so continuous activity cannot keep a cookie alive forever. Minting a session revokes that user's prior session ids. Creating a session also sweeps expired records and Orphan sessions whose `userId` no longer exists after reseed or user edit; orphan sessions are also evicted lazily on use, like expired ones. Empty or whitespace-only session cookie values are treated as absent (same as a missing cookie). When the Cookie header lists the same session name more than once, the last matching pair wins (so a stale earlier value cannot shadow a later valid session id); a trailing empty pair still clears a prior id, but a trailing malformed percent-encoded pair is skipped so it cannot wipe a prior valid session id. When Node/Fastify exposes repeated Cookie headers as a string array, the values are joined before parsing so session pairs are not dropped. Invalid bodies return `400` with `{ "error": "Invalid prototype session body", "i18nKey": "errors.invalidPrototypeSessionBody" }`. When prototype auth is off, returns `404` with `{ "error": "Prototype auth is disabled", "i18nKey": "errors.prototypeAuthDisabled" }`. |
| DELETE | `/auth/prototype-session` | Public (requires `ASSINI_ENABLE_PROTOTYPE_AUTH=true`) | Sign out of the prototype session: revokes every server-side session for the cookie's user (sibling tabs / remints) and expires the cookie with the same Secure/HttpOnly/SameSite/Path rules as create (`ASSINI_COOKIE_SECURE` / production). Returns 204 even when no session exists. When prototype auth is off, returns `404` with the same `errors.prototypeAuthDisabled` payload as POST. |
...
| PATCH | `/languages/:languageId` | reviewer, lead, admin | Update language metadata or phonology. Invalid bodies return `400` with `i18nKey: "errors.invalidLanguagePatchBody"`; unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. |
...
| POST | `/languages/:languageId/extraction-drafts/bulk-review` | reviewer, lead, admin | Accept or reject up to 50 drafts in one request with per-item results (`ok` / `error` / optional `i18nKey`+`i18nParams` / `committedEntityId`). Invalid `action` returns `400` with `i18nKey: "errors.bulkReviewInvalidAction"`; missing/empty/non-string `draftIds` returns `400` with `i18nKey: "errors.bulkReviewInvalidDraftIds"`; more than 50 ids returns `400` with `i18nKey: "errors.bulkReviewTooManyDraftIds"` and `i18nParams.max`; unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. Draft ids are trimmed before lookup. |
...
| POST | `/languages/:languageId/corpus` | reviewer, lead, admin | Import a validated corpus passage. Add `?dryRun=1` or body `dryRun: true` to validate without persisting. Invalid bodies return `400` with `i18nKey: "errors.invalidCorpusImportBody"`; semantic validation failures return `400` with `i18nKey: "errors.corpusImportValidationFailed"`; unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. |
| POST | `/languages/:languageId/corpus/bulk` | reviewer, lead, admin | Import or dry-run validate up to 50 corpus passages in one request with per-row results (`ok` / `error` / optional `i18nKey` / `warnings` / `passage` or `preview`). Add `?dryRun=1` or body `dryRun: true` to validate without persisting. Invalid bulk bodies or empty `passages` return `400` with `i18nKey: "errors.invalidCorpusBulkBody"`; more than 50 passages return `400` with `i18nKey: "errors.corpusBulkTooManyPassages"` and `i18nParams.max`; per-row schema failures use `errors.invalidCorpusImportBody`; per-row semantic failures use `errors.corpusImportValidationFailed`; unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. Partial success is allowed (`imported` / `failed` counts; HTTP `201` when any row imported, otherwise `200`). |
...
| POST | `/languages/:languageId/exercises` | reviewer, lead, admin | Author a validated exercise. Add `?dryRun=1` or body `dryRun: true` to validate without persisting. Invalid bodies return `400` with `i18nKey: "errors.invalidExerciseAuthoringBody"`; semantic validation failures return `400` with `i18nKey: "errors.exerciseAuthoringValidationFailed"`; unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. |
| POST | `/languages/:languageId/exercises/generate` | reviewer, lead, admin | Preview a grounded model-backed draft exercise (model-only, not persisted; `400` with `i18nKey: "errors.modelRequired"` without a model; grounding/model failures return `422` with `i18nKey: "errors.exerciseGenerationFailed"`). Unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. |
...
| GET | `/exports/languages/:languageId/snapshot` | reviewer, elder, lead, admin | Sanitized language snapshot with integrity metadata. Sets `Cache-Control: no-store` and a sanitized attachment filename; appends `language_snapshot.exported` audit receipt. Unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. |
| GET | `/exports/evaluations/artifact` | reviewer, lead, admin, programmer | Sanitized evaluation artifact. Sets `Cache-Control: no-store` and an attachment filename; appends `evaluation_artifact.exported` audit receipt. Empty/no-run workspaces still export with `summary.passed: false`. |
...
| GET | `/languages/:languageId/elder-context` | elder, reviewer, lead, admin | Public context and correction ledger for elder review. Unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. |
| GET | `/elder/corrections` | elder, reviewer, lead, admin | Correction records; `?languageId=` filters. Unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. |
| POST | `/elder/corrections` | elder, lead, admin | Submit a pending correction. Invalid bodies return `400` with `i18nKey: "elderWs.errInvalidCorrectionBody"`; unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`; unknown note/passage targets return `400` with `i18nKey: "elderWs.errNoteNotFoundForLanguage"` / `"elderWs.errPassageNotFoundForLanguage"`. |
...
| GET | `/observability/neural-map` | programmer, admin, lead | Role-gated sanitized context graph. Missing, blank, whitespace-only, or repeated `languageId` query values return `400` with `i18nKey: "errors.missingLanguageId"`; unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`. Programmers see elder-correction nodes with redacted labels; lead/admin keep the correction text. |
...
Prototype sessions use a sliding TTL (`ASSINI_PROTOTYPE_SESSION_TTL_MS`, default 8 hours) capped by an absolute lifetime from `createdAt` (`ASSINI_PROTOTYPE_SESSION_ABSOLUTE_MAX_MS`, default 24 hours / 3× TTL). On each successful authenticated use, the server slides `expiresAt` and re-issues `Set-Cookie` with `Max-Age` equal to the remaining lifetime (sliding ∩ absolute), so browsers do not keep a full TTL after the absolute deadline is near. Past either deadline, gated routes return `401` and expire the cookie with `Max-Age=0`.
...
`POST /languages/:languageId/sources/obsidian-vault` imports Markdown files from a local Obsidian vault folder path. The body is `{ "vaultPath": "...", "includeSubfolders": true, "maxFiles": 100 }`. The vault path must resolve under `ASSINI_OBSIDIAN_VAULT_ROOTS` (semicolon-separated absolute roots); when that env is unset/empty, only relative roots were configured, or the path is outside the allowlist (including `..` / symlink escapes), the route returns `400` with a matching `i18nKey` (`ingest.errorVaultRootsUnset`, `ingest.errorVaultRootsMustBeAbsolute`, `ingest.errorVaultOutsideAllowlist`, `ingest.errorVaultNotDirectory`, or `ingest.errorVaultUnreadable`). Relative root segments are dropped so the process CWD cannot silently widen the allowlist. The server reads `.md` files, skips `.obsidian`, `.git`, and `node_modules`, strips common frontmatter and wikilinks, and stores each non-empty note as a pending `text` source. Individual Markdown notes larger than 1 MB are skipped with reason `Markdown file is larger than the 1 MB import limit.` (localized as `ingest.vaultMarkdownTooLarge`). The response reports imported assets, skipped files with reasons, warnings, and counts. Process the imported sources normally to produce extraction drafts.
...
Each claim increments `processingAttempts` and stamps `processingStartedAt` / `processingHeartbeatAt` on the asset (heartbeats continue while the job runs). When processing finishes, in-flight markers are cleared. A successful run also clears `processingAttempts`; failures and startup recovery keep the counter. After 5 failed or abandoned claims, further `POST /sources/:sourceId/process` calls return `409` with `i18nKey: "ingest.sourceMaxProcessingAttempts"` — the capped asset stays blocked until operators inspect it or register a fresh source (retrying the same id keeps returning `409`).
...
Only `proposed` drafts can be reviewed; re-reviewing returns `400` with a status-specific `i18nKey` (`errors.extractionDraftAlreadyAccepted` / `AlreadyRejected` / `AlreadyProposed` / `AlreadyStatus`). Unknown draft ids return `404` with `{ "error": "Extraction draft not found: …", "i18nKey": "errors.extractionDraftNotFound" }`. Accept validation failures (missing fields, duplicates, phonology) also return `400` with matching `i18nKey` values.

Accept may include an optional JSON body for corpus-passage segmentation conflict resolution (Build reviewers): `{ "preferLexiconSegmentation": true }` commits lexicon longest-match segmentation, or `{ "morphologicalSegmentation": [...] }` commits a reviewer-edited morpheme list. The two fields are mutually exclusive; either on a non-corpus draft, or a malformed body, returns `400` with `i18nKey: "errors.invalidExtractionDraftAcceptBody"`. An empty/missing body keeps the draft segmentation (subject to the usual coverage fallback).

`POST /languages/:languageId/extraction-drafts/bulk-review` accepts or rejects up to 50 drafts in one request. Draft ids are trimmed and deduplicated. The response is `{ results, accepted, rejected, failed }` where each result is `{ draftId, ok, error?, i18nKey?, i18nParams?, committedEntityId? }`. Per-item failures (missing draft, wrong language, already reviewed, accept validation) do not abort the rest of the batch.
...
The route imports one corpus passage after validating provenance, consent, segmentation, and duplicate target text. The response returns only the public corpus passage, but the server also stores a private corpus answer key derived from the validated target text, translation, and segmentation. Bodies that fail schema parsing return `400` with `{ "error": "Invalid corpus import body", "i18nKey": "errors.invalidCorpusImportBody" }`. Semantic validation failures (segmentation, phonology, lexicon grounding, duplicates) return `400` with `i18nKey: "errors.corpusImportValidationFailed"`. Unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`.
...
`POST /languages/:languageId/corpus/bulk`

Bulk import accepts `{ "passages": [ /* same fields as single corpus import */ ] }` (optional top-level `dryRun: true`, or `?dryRun=1`). Up to 50 passages are validated independently; later rows see earlier in-batch successes so duplicate target text is caught within the request. The response is `{ "ok": boolean, "dryRun": boolean, "imported": number, "failed": number, "results": [...] }` where each result is `{ index, ok, warnings, passage? | preview?, error?, i18nKey? }`. Invalid or empty bulk bodies return `400` with `i18nKey: "errors.invalidCorpusBulkBody"`; oversized batches return `400` with `i18nKey: "errors.corpusBulkTooManyPassages"` and `i18nParams.max`. Successful non-dry-run rows create the same `corpus.imported` audit events as single import (with `metadata.bulk: true`). Partial success does not roll back earlier rows.

## Exercise authoring

`POST /languages/:languageId/exercises`

The route stores private answer-key fields server-side and returns only the public exercise shape. Bodies that fail schema parsing return `400` with `{ "error": "Invalid exercise authoring body", "i18nKey": "errors.invalidExerciseAuthoringBody" }`. Semantic validation failures (unknown rules/vocabulary, duplicates, underspecified prompts/explanations, adversarial probes) return `400` with `i18nKey: "errors.exerciseAuthoringValidationFailed"`. Unknown language ids return `404` with `i18nKey: "errors.languageNotFound"`.
...
Language snapshots (`language-snapshot-v2`) and evaluation artifacts (`evaluation-artifact-v2`) include SHA-256 integrity manifests. The `integrity.contentHash` is SHA-256 over a stable-key-order JSON serialization of the export payload with the `integrity` field omitted (`generatedBy: assini-local-export-v1`). Reviewers can recompute that hash offline—or use `verifyExportIntegrity` in `publicLanguageViews.ts`—to confirm a downloaded file was not altered after export. `verifyExportIntegrity` rejects manifests with a missing or null `integrity`, a non-object `integrity` value, integrity fields inherited from a prototype (own properties required), an unexpected or missing `algorithm`/`generatedBy`, a mismatched, reordered, truncated, or extended `redactionPolicy` list (including non-array values), a missing, non-string, or unknown `exportVersion` (only `language-snapshot-v2` and `evaluation-artifact-v2` are accepted), a non-string or non-hex `contentHash` (exactly 64 hex digits; case-insensitive), or a hash that does not match the recomputed payload. Payload key order does not affect verification. Snapshots carry the state-derived linguistic profile: phonology, lexicon vocabulary, derived morpheme inventory, grammar rules, and stats including source-asset and pending-draft counts. Evaluation artifacts include latest runs, run histories by language, trend records with category deltas, gate metadata, and failure lines. An empty workspace (no languages) or a workspace with no recorded evaluation runs exports `summary.passed: false` with a System Eval guidance failure line rather than a vacuous green gate. Both omit private fields such as answer keys, adversarial probes, learner answers, learner submissions, AI sessions, local users, provider prompts, and hidden model traces. Successful export responses set `Cache-Control: no-store` / `Pragma: no-cache` and a sanitized `Content-Disposition: attachment` filename, and append an audit receipt (`language_snapshot.exported` or `evaluation_artifact.exported`) with the integrity hash—not private payload fields. `GET /exports/languages/:languageId/snapshot` returns `404` with `{ "error": "Language not found: …", "i18nKey": "errors.languageNotFound" }` when the language id is unknown (no audit receipt).

## Evaluation runs

`POST /evaluations/run`

Evaluation runs are generated for existing languages against the workspace corpus (`fixtureVersion: "workspace-corpus-v1"`). When the workspace has no languages, the route returns `400` with `{ "error": "No languages available to evaluate. Create a language from the sidebar first, then run System Eval.", "i18nKey": "errors.noLanguagesToEvaluate" }` and does not append runs. The CLI (`npm run eval`) exits `0` with empty-workspace guidance on stdout (create a language, then re-run) unless `ASSINI_EVAL_REQUIRE_LANGUAGES` is set (verify sets this so an empty seed cannot green-pass). Persisted runs are validated during local JSON reads: each run must keep a nonblank language ID that references an existing language, nonblank version and summary text, a parseable `createdAt`, and failure lines that match their parent run's language.
