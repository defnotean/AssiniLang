...
| `GET /languages/:languageId/profile` | Return public linguistic profile data for one language, including phonology, vocabulary, semantic domains, register profiles, grammar, paradigms, dialect variants with evidence-backed history timelines, discourse examples, teaching sequences, a derived morpheme inventory, the shared synthetic fixture quality floor, and actual-vs-minimum quality checks. |
...
Successful imports create a `corpus.imported` audit event. The persisted app-state schema later verifies that corpus passages still keep nonblank language IDs that reference existing languages, keep nonblank source/provenance text, target text, translations, consent restrictions, topic tags, morpheme surface/lemma/gloss text, and morpheme feature labels, keep normalized topic tags and morpheme feature labels duplicate-free, and preserve target-text segmentation coverage. It also verifies that corpus answer keys still keep nonblank passage and language IDs, reference existing same-language passages, keep nonblank target text, translations, morpheme text, and feature labels, keep morpheme feature labels duplicate-free, and preserve their own segmentation coverage so manually edited local JSON cannot leave evaluation keys orphaned, malformed, or attached to the wrong language.
...
During local JSON reads, restored notes and note answer keys must keep nonblank language IDs that reference existing languages, keep topic, explanation, and dialect scope text nonblank, keep `evidenceCount` aligned with `evidencePassageIds`, keep reviewer and edit-history timestamps parseable when present, use nonblank review-capable local users or reserved deterministic system actors for reviewer and edit-history attribution, keep reviewer comments and edit-history summaries nonblank, keep edit-history actions inside the known note timeline vocabulary (`created`, `drafted`, `migrated`, `reviewed`, `disposition_resolved`, and `applied_correction`), cite nonblank same-language evidence passages, and keep examples linked by nonblank corpus passage IDs synchronized with the cited corpus target text and translation.
...
The persisted app-state schema enforces the same review-policy assignment, threshold, nonblank language, nonblank updater attribution, and parseable update-timestamp rules, so malformed local JSON cannot create impossible approval quorums or policies detached from a real synthetic language. Restored policies are unique per language and must be updated by a known lead/admin user, except seeded policies marked with `system-seed`.
...
Language snapshots and evaluation artifacts include SHA-256 integrity manifests. Language snapshots carry the public profile's `fixtureMinimums` object and `fixtureQuality` summary so reviewers can compare exported data against the current synthetic depth floor without exposing answer keys. Public profile data includes semantic domains with labels, descriptions, core vocabulary IDs, usage notes, and public corpus evidence IDs. It includes register profiles with IDs, labels, usage context, style labels, semantic-domain IDs, discourse-example IDs, teaching-sequence IDs, public corpus evidence IDs, and usage notes. It includes dialect histories with summaries, period labels, descriptions, and public corpus evidence IDs. It also includes discourse examples with function labels, usage context, target text, translation, and pragmatic notes. Teaching sequences include an ID, title, objective, intro/practice/review level, referenced grammar-rule IDs, referenced corpus-passage IDs, referenced public exercise IDs, and ordered step prompts. The snapshot `fixtureQuality` summary includes aggregate `totalChecks`, `passedChecks`, and `failedChecks` counters plus per-check actual/minimum rows, including semantic-domain, register-profile, dialect-history, discourse-example, and teaching-sequence coverage. Evaluation artifacts include aggregate fixture-quality readiness in `summary.fixtureQuality`, with language counts, passed/failed language counts, total check counts, passed check counts, failed check counts, and an overall pass flag. They omit private fields such as answer keys, learner answers, learner submissions, AI sessions, local users, provider prompts, and hidden model traces.

## Evaluation Runs

`POST /evaluations/run`

Evaluation runs are generated for existing synthetic languages. Persisted runs are validated during local JSON reads: each run must keep a nonblank language ID that references an existing language, keep nonblank system version, fixture version, score category, and summary text, keep a parseable `createdAt`, and every stored failure line must use a nonblank language ID matching its parent run with nonblank category, item ID, and diagnostic message.
