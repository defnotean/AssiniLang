...
To exercise the whole flow in one command against a running dev server, `node scripts/setupKelevi.mjs` builds "Kelevi" - a fully synthetic agglutinative demo language - end to end: provider health check, language creation with a CV phonology inventory, wordlist and text ingestion (model-backed when `ASSINI_LLM_*` is configured), bulk draft review, validated segmented corpus imports, model-drafted grammar notes with grounding scores, exercise authoring and grading, spaced-repetition recommendations, and an evaluation run. It needs `ASSINI_DEV_AUTH_TOKEN` to match the running API (defaults to `dev-local`, same as the other local driver scripts) and resets nothing by itself; run `npm.cmd run seed` first for a clean slate.
