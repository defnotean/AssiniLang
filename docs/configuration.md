...
| `ASSINI_PROTOTYPE_SESSION_TTL_MS` | `28800000` (8 hours) | positive integer (milliseconds) | Prototype session sliding lifetime. Each successful use within the TTL slides the server `expiresAt` forward (capped by `ASSINI_PROTOTYPE_SESSION_ABSOLUTE_MAX_MS`) and re-issues `Set-Cookie` with a `Max-Age` matching the **remaining** lifetime (sliding ∩ absolute), not a full TTL reset when the absolute deadline is nearer; expired, orphaned, or unknown session cookies return 401, are evicted lazily on use and opportunistically during session creation (expired + missing-user orphans), and the 401 response expires the browser cookie with `Max-Age=0` so clients stop replaying a dead session id. Opening a new session for a user revokes that user's prior session ids; logout revokes every session for the cookie's user. Empty or whitespace-only session cookie values (for example after sign-out `Max-Age=0`) are treated as absent. When the Cookie header lists the same session name more than once, the last matching pair wins; a trailing empty pair still clears a prior id, but a trailing malformed percent-encoded pair is skipped so it cannot wipe a prior valid session id. Repeated Cookie headers exposed as a string array are joined before parsing so session pairs are not dropped. Invalid values fail server startup. |
| `ASSINI_PROTOTYPE_SESSION_ABSOLUTE_MAX_MS` | `86400000` when TTL is default; otherwise `3×` the configured sliding TTL | positive integer (milliseconds), must be ≥ sliding TTL | Hard cap on total prototype-session age from `createdAt`. When unset, defaults to 24 hours for the default 8-hour TTL, or three times the configured sliding TTL if you customized `ASSINI_PROTOTYPE_SESSION_TTL_MS`. Sliding renewal cannot extend a session past this deadline, so continuous activity cannot keep a cookie alive forever. Invalid values or a value below the sliding TTL fail server startup. |
