...
| `ASSINI_PROTOTYPE_SESSION_TTL_MS` | `28800000` (8 hours) | positive integer (milliseconds) | Prototype session lifetime. Each successful use within the TTL slides the server `expiresAt` forward and re-issues `Set-Cookie` with a matching `Max-Age` so browsers keep the cookie; expired sessions return 401 and are evicted lazily and during session creation. Empty or whitespace-only session cookie values (for example after sign-out `Max-Age=0`) are treated as absent. Invalid values fail server startup. |
