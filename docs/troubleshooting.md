...
| `npm run db:backup` fails saying the destination is a directory | The path you passed is an existing folder, not a backup file | Pass a file path (for example `data/backups/my-backup.json`) or omit the path to use the timestamped default under `data/backups/`. |
| `npm run db:backup` fails saying the destination already exists | An earlier backup file is already at that path; overwrite is refused by default | Pass a new file path, or add `--force` to overwrite deliberately. |
| `JsonStore.restoreFrom` fails saying the backup source is a directory | Restore was pointed at a folder instead of a backup database file | Pass the backup **file** path (for example `data/backups/local-db-TIMESTAMP.json`), not the `data/backups/` folder. Desktop restore uses a backup folder that contains `data/`; CLI restore expects the database file itself. |
