...
| Model Setup view shows warnings like `Local/OpenAI-compatible mode requires ASSINI_LLM_BASE_URL.` | `ASSINI_LLM_PROVIDER` is set but its required companions are missing | Set `ASSINI_LLM_BASE_URL` and `ASSINI_LLM_MODEL` (local modes) or an API key (remote mode) on the API process and restart. |
