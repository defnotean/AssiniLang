...
The implemented baseline should keep at least 24 public vocabulary items, 12 corpus passages, 6 grammar rules, 6 note answer keys, and 6 learner exercises per synthetic language. Each language should also include at least two exercise types so the grading workflow is not trained only on a single task shape.
...
- Each synthetic language has at least 24 public vocabulary items, 12 corpus passages, 6 grammar-derived note answer keys, and 6 learner exercise answer keys.
