...
Persisted corpus passages must reference existing synthetic languages; keep nonblank source labels, source author, source license, source consent record, target text, translation, consent restrictions, morpheme surfaces, morpheme lemmas, and morpheme glosses; keep at least one nonblank topic tag; keep duplicate-free topic tags and morpheme feature lists after whitespace normalization; use segmentation surfaces that appear in the target text; and cover every target-text token with one or more contiguous segmentation surfaces. The persisted DB layer does not check phonology inventory or vocabulary grounding because those depend on synthetic fixture metadata; those checks stay in `packages/synthetic-langs` and the live corpus import route.
