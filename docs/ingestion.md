...
| `Source processing attempt limit reached (5).` (`i18nKey: ingest.sourceMaxProcessingAttempts`) | 409 | Five failed or abandoned claims (`processingAttempts` ≥ 5); successful runs reset the counter | Inspect the asset error/history, fix the underlying failure, or register a fresh source; do not keep retrying the same asset. |
