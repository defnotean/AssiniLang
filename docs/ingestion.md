...
A source that is already `processing` returns `409` in both modes. There is no in-process resume: if the server crashes mid-processing the asset stays `processing` (see [troubleshooting](troubleshooting.md)).
