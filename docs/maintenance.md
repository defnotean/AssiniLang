...
`JsonStore` exposes `backupTo(destinationPath)` and `restoreFrom(sourcePath)` for both backends: JSON backups are byte-for-byte copies; SQLite backups use better-sqlite3's online backup API. Both operate on the **database file only** (not `data/assets/`); Desktop folder backups include the data directory when you need uploaded assets restored with the workspace. Both refuse when the source and destination resolve to the same path, including when one path is a symlink alias of the other (`realpath` identity), a hard-link alias (same device + inode), a Windows case-only path alias of the live file, or a `\\?\` extended-length prefix of the same path (`--force` does not bypass same-file identity). Destination must be a **file** path (not an existing directory); `npm run db:backup` and `backupTo` fail with a clear message if you pass a folder. An existing destination **file** is also refused unless `backupTo(..., { force: true })` / CLI `--force` is set, so operators cannot silently overwrite a prior backup (JSON and SQLite both overwrite the destination when force is set). Restore likewise refuses when the backup source is an existing directory (same clear file-path error as backup). `npm run db:backup` validates the live workspace against the current schema before copying, so a corrupt database cannot be archived as a backup. Restore also validates that the backup parses against the current schema before it replaces the live database, and fails loudly (with the database path) otherwise. AssiniLang Desktop create validates the live workspace the same way before writing a backup folder; Desktop restore validates the backup database the same way, then creates a restorable safety backup and refuses to replace live data if that safety backup fails. Desktop "restore latest" prefers the newest routine backup over a newer safety-before-restore copy so a second restore does not undo the first.
