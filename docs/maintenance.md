...
Routes live in `apps/api/src/server.ts` inside `createServer`. Use `POST /languages/:languageId/sources` (the source-registration route) as the template - it is short and shows the full pattern:
...
The console is one app in `apps/web/src/App.tsx`:

1. Extend the `ViewMode` union, add an entry to `VIEW_CONFIG` (label, title, eyebrow), and add the mode to `VIEW_ORDER`. Optionally extend `ViewGlyph` and `sectionCounts`.
2. Write the view component in `App.tsx` following the existing ones (`IngestView`, `CorpusView`, ...): take the selected `languageId`, load through an `AsyncState<T>`, render loading/error/ready states.
