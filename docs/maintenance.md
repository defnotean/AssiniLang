...
`JsonStore` exposes `backupTo(destinationPath)` and `restoreFrom(sourcePath)` for both backends: JSON backups are byte-for-byte copies; SQLite backups use better-sqlite3's online backup API. Both refuse when the source and destination resolve to the same path, including when one path is a symlink alias of the other (`realpath` identity) or a hard-link alias (same device + inode). Destination must be a **file** path (not an existing directory); `npm run db:backup` and `backupTo` fail with a clear message if you pass a folder. Restore likewise refuses when the backup source is an existing directory (same clear file-path error as backup). `npm run db:backup` validates the live workspace against the current schema before copying, so a corrupt database cannot be archived as a backup. Restore also validates that the backup parses against the current schema before it replaces the live database, and fails loudly (with the database path) otherwise. AssiniLang Desktop restore validates the backup database the same way before replacing live data.
